...
<section>
  ...
  <form class="question flex flex--column flex--gapped" action={data.form.fields.action} method={"POST"} bind:this={form} oninput={() => isValid = form.checkValidity()} onsubmit={async e => {
    ...
    <label for="input-newsletter-accept" class="flex flex--middle flex--gapped">
      ...
      <span>{#if getLocale() === 'fr'}J'accepte la <a href="/fr/docs/Politique sur la gestion des renseignements personnels" target="_blank"><u>Politique de confidentialité des données numériques</u></a> de Barrage Capital. {:else}I agree to Barrage Capital’s <a href="/docs/Privacy Policy" target="_blank"><u>Digital Data Privacy Policy.</u></a>{/if}</span>
    </label>
